...
import dev.ninemmteam.api.interfaces.IShaderEffectHook;
...
public class ShaderManager
   ...
   public void applyShader(Runnable runnable, Shader mode) {
      ...
      if (effect != null) {
         ((IShaderEffectHook)effect).fentany1Client$addHook("bufIn", this.shaderBuffer);
      } else {
         return;
      }
   ...
   public void reloadShaders() {
      ...
      DEFAULT_OUTLINE = ShaderEffectManager.getInstance().manage(Identifier.of((String)"shaders/post/outline.json"), managedShaderEffect -> {
         ...
         ((IShaderEffectHook)effect).fentany1Client$addHook("bufIn", ShaderManager.mc.worldRenderer.getEntityOutlinesFramebuffer());
         ((IShaderEffectHook)effect).fentany1Client$addHook("bufOut", ShaderManager.mc.worldRenderer.getEntityOutlinesFramebuffer());
      });
