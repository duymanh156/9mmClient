...
import dev.ninemmteam.asm.accessors.IGameRenderer;
...
public class ShaderModule
    ...
    public boolean shouldRender(Entity entity) {
        ...
        if (entity instanceof PlayerEntity) {
            ...
            if (fentanyl.FRIEND.isFriend(playerEntity)) {
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (this.hands.getValue()) {
            fentanyl.SHADER.renderShader(() -> ((IGameRenderer)ShaderModule.mc.gameRenderer).IRenderHand(ShaderModule.mc.gameRenderer.getCamera(), mc.getRenderTickCounter().getTickDelta(true), matrixStack.peek().getPositionMatrix()), this.mode.getValue());
        }
    }

    @Override
    public void onToggle() {
        fentanyl.SHADER.reloadShaders();
    }
